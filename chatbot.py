...
def login():
    ...
    try:
        ...
        if not login_identifier or not password:
            ...
            pharmacy = None
            logger.info(f"Pharmacy login attempt: identifier='{login_identifier}', role='{role}'")
            if '@' in login_identifier:
                pharmacy = Pharmacy.query.filter_by(email=login_identifier.lower()).first()
            elif login_identifier.upper().startswith('PHARM'):
                pharmacy = Pharmacy.query.filter_by(pharmacy_id=login_identifier.upper()).first()
            else:
                pharmacy = Pharmacy.query.filter_by(name=login_identifier).first()

            if pharmacy and pharmacy.is_active and pharmacy.check_password(password):
                session.permanent = True
                session['user_id'] = pharmacy.id
                session['pharmacy_id'] = pharmacy.pharmacy_id
                session['role'] = 'pharmacy'
                session['login_time'] = datetime.now().isoformat()

                create_user_session(pharmacy, {
                    'remote_addr': request.environ.get('REMOTE_ADDR'),
                    'user_agent': request.environ.get('HTTP_USER_AGENT')
                })

                logger.info(f"✅ Pharmacy login successful: {pharmacy.name} ({pharmacy.pharmacy_id})")
                return jsonify({
                    "success": True,
                    "message": f"Welcome back, {pharmacy.name}!",
                    "redirect": "/store.html",
                    "user": {
            ...
            elif pharmacy and not pharmacy.is_active:
...
def place_order():
    ...
    try:
        ...
        # Handle both string and integer pharmacy IDs
        if isinstance(pharmacy_id, int):
...
def get_pharmacy_dashboard():
    ...
    try:
        ...
        if not pharmacy_id_str:
            return jsonify({"error": "Not authenticated as pharmacy"}), 401

        pharmacy = Pharmacy.query.filter_by(pharmacy_id=pharmacy_id_str).first()
        if not pharmacy:
